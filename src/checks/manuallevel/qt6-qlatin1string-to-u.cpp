        ...
        DeclRefExpr *child_declRefExp = dyn_cast<DeclRefExpr>(child);
         ...
         if (child_dynCastExp || child_condOp) {// skipping those.
            ...
            i++;
             ...
             //llvm::outs() << "child_declRefExp Value " << child_declRefExp->getNameInfo().getAsString() << "\n";
             //llvm::outs() << "child_declRefExp Type" << child_declRefExp->getType().getAsString()<< "\n";
            return {};
        } else {
...
{
    // lucie: why do I visit all the include files? This does not seem to happen for the AST visit.
